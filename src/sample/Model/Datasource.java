...
public class Datasource {
    ...
    public static final String CONNECTION_STRING = "jdbc:sqlite:/Users/georgefuior/Desktop/JavaPrograms/Music/" + DB_NAME;

    public static final String TABLE_ALBUMS = "albums";
    public static final String COLUMN_ALBUM_ID = "AlbumId";
    public static final String COLUMN_ALBUM_NAME = "Title";
    ...
    public static final String TABLE_SONGS = "tracks";
    public static final String COLUMN_SONGS_TRACK = "TrackId";
    ...
    public static final String QUERY_ARTISTS_FOR_SONG_START = "SELECT " + TABLE_ARTISTS + "." + COLUMN_ARTIST_NAME + ", " + TABLE_ALBUMS + "." + COLUMN_ALBUM_NAME
            + " ," + TABLE_SONGS + "." + COLUMN_SONGS_TRACK + " FROM " + TABLE_SONGS + " INNER JOIN " + TABLE_ALBUMS + " ON " + TABLE_SONGS + "." + COLUMN_SONG_ALBUM +
    ...
    public static final String CREATE_ARTIST_FOR_SONG_VIEW = "CREATE VIEW IF NOT EXISTS " +
            ...
            TABLE_ALBUMS + "." + COLUMN_ALBUM_NAME + " AS AlbumTitle " + ", " + TABLE_SONGS + "." +
            COLUMN_SONGS_TRACK + ", " + TABLE_SONGS + "." + COLUMN_SONGS_TITLE + " AS SongTitle" +
            " FROM " + TABLE_SONGS +
    ...
    public static final String QUERY_VIEW_SONG_INFO = "SELECT " + COLUMN_ARTIST_NAME + ", AlbumTitle, " +
            COLUMN_SONGS_TRACK + " FROM " + TABLE_ARTIST_SONG_VIEW + " WHERE SongTitle = \"";

    public static final String QUERY_VIEW_SONG_INFO_PREP = "SELECT " + COLUMN_ARTIST_NAME + ", AlbumTitle, " + COLUMN_SONGS_TRACK +
            " FROM " + TABLE_ARTIST_SONG_VIEW + " WHERE SongTitle = ?";
    ...
    public static final String INSERT_ALBUMS = "INSERT INTO " + TABLE_ALBUMS + "(" + COLUMN_ALBUM_NAME + ", " + COLUMN_ALBUM_ARTIST +
            ") VALUES(?, ?)";
    public static final String INSERT_SONGS = "INSERT INTO " + TABLE_SONGS + "(" + COLUMN_SONGS_TRACK + ", " + COLUMN_SONGS_TITLE + ", "
            + COLUMN_SONG_ALBUM +" ,MediaTypeId, GenreId, Composer, Milliseconds, Bytes, UnitPrice) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    private static Datasource instance = new Datasource(); //instance will be created at first time to be thread safe
    private Datasource(){
        //Singleton class -> only one instance
    }
    public static Datasource getInstance(){
       // if(instance==null){ //Lazy instantiation --> not thread safe
    ...
    public boolean open() {
        try {
            ...
            insertIntoArtists = conn.prepareStatement(INSERT_ARTIST, Statement.RETURN_GENERATED_KEYS); //Get the key from the Prepare statement object
            insertIntoAlbums = conn.prepareStatement(INSERT_ALBUMS, Statement.RETURN_GENERATED_KEYS);
            insertIntoSongs = conn.prepareStatement(INSERT_SONGS); //Don't need the Id
    ...
    public void close() {
        ...
        //Atentie la ordine! Ultima data se inchide connection
        try {
    ...
    private int inserArtist(String name) throws SQLException {
        queryArtist.setString(1, name);
        ResultSet results = queryArtist.executeQuery();
        if (results.next()) {
            return results.getInt(1);
        } else {
            //Inser the artist (it is not in the file
            insertIntoArtists.setString(1, name);
            int affectedRows = insertIntoArtists.executeUpdate(); //how many rows are updated
            if (affectedRows != 1) {
            ...
            if (generatedKeys.next()) {
                ...
                throw new SQLException("Could not get ArtistId");
            }
        }
    }
    ...
    private int insertAlbum(String name, int artistId) throws SQLException {
        ...
        if (results.next()) {
            ...
            int affectedRows = insertIntoAlbums.executeUpdate(); //how many rows are updated
            if (affectedRows != 1) {
            ...
            if (generatedKeys.next()) {
                ...
                throw new SQLException("Could not get AlbumId");
            }
        }
    }
    ...
    public boolean updateArtistName(int id, String newName){
        try{
            ...
            return affectedRecords ==1; //return true only if affected records = 1;
    ...
    public void insertSong(String title, String artist, String album, int track) {
        try {
            ...
            int artistId = inserArtist(artist);
            int albumId = insertAlbum(album, artistId);
            insertIntoSongs.setInt(1, track);
            insertIntoSongs.setString(2, title);
            insertIntoSongs.setInt(3, albumId);
            insertIntoSongs.setInt(4,10);
            insertIntoSongs.setInt(5,10);
            insertIntoSongs.setString(6,"dasdas");
            insertIntoSongs.setInt(7,10);
            insertIntoSongs.setInt(8,10);
            insertIntoSongs.setInt(9,10);

            int affectedRows = insertIntoSongs.executeUpdate(); //how many rows are updated
            if (affectedRows == 1) {
                conn.commit();
            } else {
                throw new SQLException("This song inser failed");
            }
        } catch (Exception e) {
            ...
            try {
                System.out.println("Resetind  default commit behaviour");
                conn.setAutoCommit(true);
            } catch (SQLException e) {
                System.out.println("Couldn't reset auto-comit!" + e.getMessage());
            }
        }
    }
}
